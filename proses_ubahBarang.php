...
if(isset($_POST['ubah_foto'])){ // Jika user menceklis checkbox yang ada di form ubah, lakukan :
  ...
  if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
    ...
    $query = "SELECT * FROM mbarang WHERE id_barang='".$id_barang."'";
    ...
    $query = "UPDATE mbarang SET kondisi='".$kondisi."', spek='".$spek."', gambar='".$fotobaru."' WHERE id_barang='".$id_barang."'";
    $sql = mysqli_query($koneksi, $query); // Eksekusi/ Jalankan query dari variabel $query
  ...
  // Proses ubah data ke Database
  $query = "UPDATE mbarang SET  kondisi='".$kondisi."', spek='".$spek."' WHERE id_barang='".$id_barang."'";
  $sql = mysqli_query($koneksi, $query); // Eksekusi/ Jalankan query dari variabel $query
  if($sql){ // Cek jika proses simpan ke database sukses atau tidak
    // Jika Sukses, Lakukan :
    header("location: barang.php"); // Redirect ke halaman index.php
